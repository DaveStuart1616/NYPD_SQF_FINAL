...
# %% pick a stop reason stop bulge
df_bulge = df[df["cs_bulge"] == "YES"]
...
for k in tqdm(range(num_city, num_pct, 10)):
    ...
    y = c.fit_predict(df_bulge[["lat", "lon"]])
    scores[k] = silhouette_score(df_bulge[["lat", "lon"]], y)
    labels[k] = y
...
df_bulge["label"] = labels[best_k]
for r in df_bulge.to_dict("records"):
    folium.CircleMarker(
...
# %% value counts
df_bulge["label"].value_counts()

# %% pick a label and visualize the datapoints on map
biggest_cluster = df_bulge["label"].value_counts().index[1]
m = folium.Map((40.7128, -74.0060))
for r in df_bulge[df_bulge["label"] == biggest_cluster].to_dict("records"):
    folium.CircleMarker(
        location=(r["lat"], r["lon"]), radius=1, color=colors[r["label"]]
    ).add_to(m)
